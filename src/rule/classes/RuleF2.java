...
/**
 * La classe <code> RuleF2 </code> implemente la regle F2 contenue dans le
 * cahier des charges
 * 
...
public class RuleF2 implements RuleI {
	...
	public boolean filter(ArrayList<EventI> matchedEvents, CorrelatorI c) throws Exception {
		...
		fcs = (FIRECorrelator) fireState;
		return fireState.isInArea(matchedEvents.get(0).getPropertyValue("area"),
				matchedEvents.get(0).getPropertyValue("centerId"))
				&& fireState.standardTruckAvailable(matchedEvents.get(0).getPropertyValue("centerId"));
	}
