...
/**
 * La classe <code> RuleS1 </code> implemente la regle S1 contenue dans le
 * cahier des charges
 * 
...
public class RuleS1 implements RuleI {
	...
	public ArrayList<EventI> match(EventBaseI eb) {
		...
		for (int i = 0; i < eb.numberOfEvents() && (he == null); i++) {
			...
			if (e instanceof HealthEvent && e.hasProperty("type")
					&& ((String) e.getPropertyValue("type")).equals("emergency") && e.hasProperty("area")) {
				he = e;
			}
		}
		if (he != null) {
	...
	public void act(ArrayList<EventI> matchedEvents, CorrelatorI c) throws Exception {
		SAMUCorrelatorI samuState = (SAMUCorrelatorI) c;
		samuState.triggerAmbulanceIntervention(matchedEvents.get(0).getPropertyValue("area"),
				matchedEvents.get(0).getPropertyValue("centerId"));
